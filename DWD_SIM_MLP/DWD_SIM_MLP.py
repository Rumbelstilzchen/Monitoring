...
class myMLPRegressor(MLPRegressor):
    def __init__(
        ...
        *,
        ...
        self.worst_val_score = None

    def _score(self, X, y):
        """Private score method without input validation"""
        # Input validation would remove feature names, so we disable it
        y_pred = self._predict(X, check_input=False)
        score = 1 - np.abs(y_pred / y.squeeze() - 1).mean()
        # if self.worst_val_score is None:
        #     self.worst_val_score = score
        # if score < 0:
        #     score -= self.worst_val_score
        #     score /= (self.worst_val_score+1)
        # score =
        # print(score)
...
def lin_scaler_inverse(x, min_x=0, max_x=8500, clipping: tuple[int, int] = None):
    output = x * (max_x - min_x) + min_x
    if clipping is not None:
        if np.any(output < clipping[0]):
            logger.warning(
                f"values <0 found {np.sum(output < clipping[0])*100/x.shape[0]:.1f}% of times: clipping to zero"
            )
        if np.any(output > clipping[1]):
            logger.warning(
                f"values >=10000 found {np.sum(output > clipping[1])*100/x.shape[0]:.1f}% of times: clipping to 9999.9"
            )
        output = np.clip(output, a_min=clipping[0], a_max=clipping[1])
    return output
...
class DWD_SIM_MLP(Base_Parser):
    ...
    input_scaler = [
        (
            "Jahr",
            FunctionTransformer(
                func=lin_scaler, kw_args={"min_x": 2019, "max_x": 2048}
            ),
            ["Jahr"],
        ),
        (
            "age",
            FunctionTransformer(
                func=lin_scaler, kw_args={"min_x": 2019 * 12, "max_x": 2049 * 12}
            ),
            ["age"],
        ),
        ...
        (
            "cos_Wind_direction",
            FunctionTransformer(func=cos, kw_args={"period": 360}),
            ["Wind_direction"],
        ),
        (
            "sin_Wind_direction",
            FunctionTransformer(func=sin, kw_args={"period": 360}),
            ["Wind_direction"],
        ),
        (
            "Temperature",
            FunctionTransformer(func=lin_scaler, kw_args={"min_x": -15, "max_x": 35}),
            ["Temperature"],
        ),
        (
            "Windgeschw",
            FunctionTransformer(func=lin_scaler, kw_args={"min_x": 0, "max_x": 10}),
            ["Windgeschw"],
        ),
        (
            "Luftdruck",
            FunctionTransformer(func=lin_scaler, kw_args={"min_x": 950, "max_x": 1100}),
            ["Luftdruck"],
        ),
        (
            "Rad1h",
            FunctionTransformer(func=lin_scaler, kw_args={"min_x": 0, "max_x": 4000}),
            ["Rad1h"],
        ),
        (
            "Bewoelkung_L",
            FunctionTransformer(func=lin_scaler, kw_args={"min_x": 0, "max_x": 100}),
            ["Bewoelkung_L"],
        ),
        (
            "Bewoelkung_M",
        ...
        (
            "SS1",
            FunctionTransformer(func=lin_scaler, kw_args={"min_x": 0, "max_x": 3600}),
            ["SS1"],
        ),
        (
            "Humidity",
    ...
    def __init__(self, config):
        ...
        try:
            ...
            logger.info("No Old Model found - initializing new one")

            regressor = myMLPRegressor(
                hidden_layer_sizes=(16 * 17 // 2, 100, 32, 16),
                max_iter=100000,
                warm_start=True,  # batch_size=10000,
                n_iter_no_change=400,
                early_stopping=True,
                validation_fraction=1 - self.training_proportion,
            )
            ...
            t_regressor = TransformedTargetRegressor(
                ...
                transformer=FunctionTransformer(
                    ...
                    inv_kw_args={"max_x": 8500, "clipping": (0, 10000)},
                ),
            )
            # define transform
            transformer = ColumnTransformer(
                transformers=self.input_scaler, remainder=MinMaxScaler()
            )
            # define pipeline
    ...
    def AI_convert_dict_to_scaled_numpy(self, data_set):
        ...
        # Years
        logging_data_numpy = np.vstack(
            (logging_data_numpy, [float(f[0:4]) for f in data_set["TIMESTAMP"]])
        )
        # Month
        logging_data_numpy = np.vstack(
            (logging_data_numpy, [float(f[5:7]) for f in data_set["TIMESTAMP"]])
        )
        # hours = np.array([int(f[11:13]) for f in data_set['TIMESTAMP']])
        logging_data_numpy = np.vstack(
            (logging_data_numpy, [float(f[11:13]) for f in data_set["TIMESTAMP"]])
        )
        # age
        logging_data_numpy = np.vstack(
            (logging_data_numpy, logging_data_numpy[1] * 12 + logging_data_numpy[2])
        )
        # Temperatur
    ...
    def AI_predict_values(self, data_set):
        column_names = ["Jahr", "Monat", "Stunde", "age"]
        column_names.extend(self.value_list)
        logging_data_pandas = pd.DataFrame(
            self.AI_convert_dict_to_scaled_numpy(data_set), columns=column_names
        )
    ...
    def average_parsed_data(self):
        ...
        temp_data["TIMESTAMP"] = self.parsed_data["TIMESTAMP"][
            next(
                i_index
                for i_index, v in enumerate(item_selector_for_identical_times)
                if v
            )
        ]
        is_nan_map = None
    ...
    def create_training_validation_set(self, X, Y):
        X_hom, Y_hom = self.homogenise_data_sets(X, Y)
        X_train, X_test, y_train, y_test = train_test_split(
            X_hom, Y_hom, train_size=self.training_proportion
        )
        return {
            "X_train": X_train,
            "y_train": y_train,
    ...
    def load_trainings_data(self, last_NR_data_to_ignore=0):
        ...
        sql_str = (
            ...
            f" Year({MYSQL_config['mysql_table']}.TIMESTAMP) * 12 + month({MYSQL_config['mysql_table']}.TIMESTAMP) as age,  \n"
        ...
        data_dict.update(
            self.create_training_validation_set(
                data_dict["X_data"], data_dict["Y_data"]
            )
        )

        data_as_pandas = {
            key: pd.DataFrame(value, columns=names[1:])
            for key, value in data_dict.items()
            if key[0].upper() == "X"
        }
        data_dict.update(data_as_pandas)

        return data_dict

    def train_model(self, update_history=False, last_NR_data_to_ignore=10 * 60):
        new_model_found = False
        # get old_data
        input_data = self.load_trainings_data(
            last_NR_data_to_ignore=last_NR_data_to_ignore
        )

        try:
            with open(self.AI_model_path, "rb") as f:
                model = pickle.load(f)
            score_old = self.AI_model.score(
                input_data["X_data_raw"], input_data["Y_data_raw"]
            )
        except Exception:
            model = self.AI_model
            score_old = -1
    ...
    def explain_model(self, output_dir=None):
        ...
        X_train_summary = DenseDataWithIndex(
            X_train_summary.data,
            X_train_summary.group_names,
            list(range(X_train_summary.data.shape[0])),
            "index",
            None,
        ...
        explainer = shap.KernelExplainer(
            self.AI_model.predict, X_train_summary, keep_index=True
        )
...
if __name__ == "__main__":
    ...
    with keep.presenting() as m:
        if not m.active:
            logger.error("Error Setting keep awake")
        try:
            parser_init = DWD_SIM_MLP(configuration)

            outdir = os.path.join(
                parser_init.current_directory,
                "Model_History",
            ...
            for i in range(20):
                ...
                if parser_init.train_model(update_history=True):
                    ...
                    loss_curve = (
                        np.array(
                            parser_init.AI_model.steps[1][1].regressor_.loss_curve_
                        )
                        * 100
                    )
                    val_curve = np.array(
                    ...
                    best_val_score = parser_init.AI_model.steps[1][
                        1
                    ].regressor_.best_validation_score_
                    best_val_iter = (
                        val_curve.shape[0]
                        - parser_init.AI_model.steps[1][1].regressor_.n_iter_no_change
                        - 2
                    )
                    ...
                    fig, ax = plt.subplots(
                        figsize=(1.5 * max_display + 1, 0.8 * max_display + 1)
                    )
                    ax.plot(val_curve, label="Score (Validation)")
                    ax.hlines(best_val_score, 0, val_curve.shape[0], colors="red")
                    ax.vlines(best_val_iter, 0, 1, colors="red")
                    ax.set_ylim(max(0, val_curve.min()), min(1, val_curve.max()))
                    ax.grid()
                    ax.set_xlabel("Iteration")
                    ax.set_ylabel("Score")
                    ax2 = ax.twinx()
                    ax2.plot(loss_curve, color="green", label="Loss (test)")
                    ax2.set_ylabel("Loss")
                    ax2.set_ylim(max(0, loss_curve.min()), min(100, loss_curve.max()))
                    ...
                    iter_curve = np.arange(loss_curve.shape[0])
                    loss_at_best_val = loss_curve[best_val_iter]
                    plt.figure(figsize=(1.5 * max_display + 1, 0.8 * max_display + 1))
                    plt.subplot(121)
                    plt.scatter(loss_curve, val_curve, c=iter_curve, marker=".", s=0.5)
                    plt.scatter(
                        loss_at_best_val, best_val_score, marker="x", c="red", s=10
                    )
                    plt.grid()
                    cbar = plt.colorbar()
                    ...
                    plt.subplot(122)
                    mask = loss_curve < loss_at_best_val * 2
                    plt.scatter(
                        loss_curve[mask],
                        val_curve[mask],
                        c=iter_curve[mask],
            ...
            if update_history_flag:
                ...
                copyfile(
                    os.path.join(parser_init.current_directory, "MLP_model.bin"),
                    os.path.join(outdir, "MLP_model.bin"),
                )
                copyfile(
                    os.path.join(parser_init.current_directory, "DWD_SIM_MLP.py"),
                    os.path.join(outdir, "DWD_SIM_MLP.py"),
                )
                parser_init.explain_model(outdir)
            else:
                parser_init.simulate_old_data()
                logger.info("as old data was not updated during training - done now")
        except Exception:
