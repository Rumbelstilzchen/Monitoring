...
class BYD_Cell_voltage(Base_Parser):
    ...
    site_with_voltage_data = 'RunData'
    detail_struct = {
    ...
    def __init__(self, config):
        ...
        self.base_url = f"http://{self.configuration[self.name]['IPAdresse']}"

        self.module_url = f"{self.base_url}/goform/SetRunData"

        self.load_basicdata_from_url()
    ...
    def load_basicdata_from_url(self):
        data = {}
        for key, value in self.base_site_struct.items():
            url = f"{self.base_url}/asp/{key}.asp"
            ...
            for entry, searchstring in value.items():
                match = re.findall(searchstring[0]+r'.*\n.*\"text_l\">(?P<val>[0-9])</td', page)[0]
                if not match:
    ...
    def load_voltage_data(self):
        ...
        for array_ind in range(self.basic_data_infos['Nr_Arrays']):
            for module_ind in range(self.basic_data_infos['Nr_Modules']):
                current_data = self.parse_module_page(array_ind, module_ind)
    ...
    def parse_module_page(self, array_ind, module_ind):
        data = {'time_sec': [],
                'Array': [],
                'Module': [],
                'Cell': [],
        ...
        r4 = self.session.post(self.module_url, auth=self.auth, data=payload, timeout=self.timeout)
        ...
        for i in range(16):
            ...
            if not match:
                raise ValueError(f"No number found")
            current_data = float(match)
