...
class Luxtronik:
    ...
    def __init__(self, config):
        ...
        self.IDs = {
            10: ['flowTemperature', 0.1],
            11: ['returnTemperature', 0.1],
            12: ['returnTemperatureTarget', 0.1],
            13: ['returnTemperatureExtern', 0.1],
            15: ['ambientTemperature', 0.1],
            16: ['averageAmbientTemperature', 0.1],
            17: ['hotWaterTemperature', 0.1],
            18: ['hotWaterTemperatureTarget', 0.1],
            19: ["heatSourceIN", 0.1],
            20: ["heatSourceOUT", 0.1],
            44: ['compressor1', None],
            39: ['heatingSystemCircPump', None],
            38: ['hotWaterSwitchingValve', None],
            151: ["counterHeatQHeating", 0.1],
            152: ["counterHeatQHotWater", 0.1],
            155: ["flowRate", None],
        }
        ...
        for key, value in self.IDs.items():
            if value[0] == 'flowRate':
                ...
                if value[1] is not None:
                    self.parsed_data[value[0]] = round(raw_data[key] * value[1], 1)
                else:
                    self.parsed_data[value[0]] = raw_data[key]
        self.parsed_data['counterHeatQTotal'] = \
            self.parsed_data['counterHeatQHeating'] + self.parsed_data['counterHeatQHotWater']
...
def read_data():
    ...
    for key, value in parser_init.collect_data().items():
        print('%s:\t%s' % (key, value))
    # print(parser_init.collect_data())
    # print(parser_init.collect_data())
