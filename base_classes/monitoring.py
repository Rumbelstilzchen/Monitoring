...
class Monitoring:
    ...
    def start(self):
        ...
        waiting = (
            (self.writerate - (datetime.now().timestamp() % self.writerate))
            + self.refreshrate
            - 2
        )
            ...
            while self.running:
                ...
                for i in range(self.samples_per_write):
                    ...
                    if i == 0:
                        waiting = (
                            (
                                self.writerate
                                - (datetime.now().timestamp() % self.writerate)
                            )
                            + self.refreshrate
                            - 1
                        )
                    ...
                    time.sleep(
                        self.refreshrate
                        - (datetime.now().timestamp() % self.refreshrate)
                    )
                    ...
                    if self.data_parser is not None:
                        ...
                        if "TIMESTAMP" in self.data_parser.parsed_data.keys():
                            logger.debug(
                                "\t parsed_data: %s"
                                % str(self.data_parser.parsed_data["TIMESTAMP"])
                            )
                        else:
                            logger.debug(
                                "\t parsed_data: %s"
                ...
                if len(logging_list) > 0:
                    ...
                    if len(logging_list) > 1:
                        logging_data = self.average_of_dicts(
                            logging_list,
                            self.data_parser.average_ignores,
    ...
    def average_of_dicts(
        input_dict, list_of_ignores, list_of_zero_suppress, decimal_dict: dict
    ):
        ...
        for k in input_dict[0].keys():
            ...
            if k in list_of_ignores:
                ...
                if len(value_list) == 0:
                    ...
                    if any(t[k] == 0 for t in input_dict):
                        logger.warning(
                            f"Zero suppressed for {k}, {current_length} values used for average"
                        )
            else:
